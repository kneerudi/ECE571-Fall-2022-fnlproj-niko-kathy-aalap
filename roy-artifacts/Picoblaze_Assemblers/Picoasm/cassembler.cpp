...
const char *instructions[] = {
    "ADD",     "ADDCY",  "AND",  "CALL", "COMPARE", "DISABLE", "ENABLE",
    "FETCH",   "INPUT",  "JUMP", "LOAD", "OR",      "OUTPUT",  "RETURN",
    "RETURNI", "ROTATE", "RL",   "RR",   "SL0",     "SL1",     "SLA",
...
void CAssembler::error(unsigned int line, const char *description) {
    ...
    //     QListViewItem *item = new QListViewItem( m_messageList,
    //     m_messageList->lastChild() ) ;
...
bool CAssembler::buildSymbolTable( ) {
	...
	if (name == "NAMEREG") {
	    if (!(*it)->isColumn(3)) {
		error((*it)->m_lineNr,
		      "'NAMEREG register_name, new_name' expected");
		return FALSE;
	    }
	    ...
	    for (it1 = m_registerTable.begin( ); it1 != m_registerTable.end( );
	         it1++) {
		if ((*it1)->reg == (*it)->getColumn(1)) {
		    error((*it)->m_lineNr,
		          string("NAMEREG  reg (" + (*it)->getColumn(1) +
		                 ") already aliased.")
		              .c_str( ));
		    return FALSE;
		}
		if ((*it1)->name == (*it)->getColumn(3)) {
		    error((*it)->m_lineNr,
		          string("NAMEREG alias (" + (*it)->getColumn(3) +
		                 ") already used.")
		              .c_str( ));
		    return FALSE;
		}
	    ...
	    if ((ckname[0] == 's' || ckname[0] == 'S') &&
	        ckname.length( ) == 2) {
		if ((ckname[1] >= '0' && ckname[1] <= '9') ||
		    (ckname[1] >= 'a' && ckname[1] <= 'f') ||
		    (ckname[1] >= 'A' && ckname[1] <= 'F')) {
		    error((*it)->m_lineNr, "ILLEGAL CONSTANT: constant cannot "
		                           "be a register name s[0-f]");
		    return FALSE;
		}
	    }
	    ...
	    for (ita = m_constantTable.begin( ); ita != m_constantTable.end( );
	         ita++) {
		if ((*ita)->name == (*it)->getColumn(1)) {
		    error((*it)->m_lineNr,
		          string("CONSTANT (" + (*it)->getColumn(1) +
		                 ") already defined.")
		              .c_str( ));
		    return FALSE;
		}
	    ...
	    for (it2 = m_labelTable.begin( ); it2 != m_labelTable.end( );
	         it2++) {
		if ((*it2)->name == (*it)->getColumn(0)) {
		    error((*it)->m_lineNr, string("Duplicate Lable (" +
		                                  (*it)->getColumn(0) + ":).")
		                               .c_str( ));
		    return FALSE;
		}
...
bool CAssembler::addInstruction(instrNumber instr, CSourceLine sourceLine,
                                int offset) {
	...
	case RETURN:
		...
		switch (instr) {
			...
			break;
		    default:
			error(line, "'CALL', 'JUMP' or 'RETURN' expected");
			return FALSE;
		}
	...
	default:
	    ...
	    switch (instr) {
		case RL:
		    code = instrRL_SX | instrROTATE | (reg << 8);
		    break;
		case RR:
		    code = instrRR_SX | instrROTATE | (reg << 8);
		    break;
		case SL0:
		    code = instrSL0_SX | instrROTATE | (reg << 8);
		    break;
		case SL1:
		    code = instrSL1_SX | instrROTATE | (reg << 8);
		    break;
		case SLA:
		    code = instrSLA_SX | instrROTATE | (reg << 8);
		    break;
		case SLX:
		    code = instrSLX_SX | instrROTATE | (reg << 8);
		    break;
		case SR0:
		    code = instrSR0_SX | instrROTATE | (reg << 8);
		    break;
		case SR1:
		    code = instrSR1_SX | instrROTATE | (reg << 8);
		    break;
		case SRA:
		    code = instrSRA_SX | instrROTATE | (reg << 8);
		    break;
		case SRX:
		    code = instrSRX_SX | instrROTATE | (reg << 8);
		    break;
		default:
		    if (s2 != ",") {
			error(line, "Comma expected");
			...
			case FETCH:
				...
				if (!sourceLine.isColumn(offset + 5) ||
				    sourceLine.getColumn(offset + 5) != ")") {
				    error(line, "')' expected");
				    return FALSE;
				}

				int reg2 = getRegister(translateRegister(
				    sourceLine.getColumn(offset + 4)));
				if (reg2 < 0) {
				...
				switch (instr) {
				    case STORE:
					code = instrSTORE_SX_SY | (reg << 8) |
					       (reg2 << 4);
					break;
				    case OUTPUT:
					code = instrOUTPUT_SX_SY | (reg << 8) |
					       (reg2 << 4);
					break;
				    case INPUT:
					code = instrINPUT_SX_SY | (reg << 8) |
					       (reg2 << 4);
					break;
				    case FETCH:
					code = instrFETCH_SX_SY | (reg << 8) |
					       (reg2 << 4);
					break;
				    default:
					error(line,
					      "'STORE', 'OUTPUT', 'INPUT' or "
					      "'FETCH' expected");
					return FALSE;
				}
			    } else {
				...
				unsigned int value;
				if (sscanf(translateConstant(s3).c_str( ), "%X",
				           &value) != 1) {
				...
				switch (instr) {
				    case STORE:
					code = instrSTORE_SX_SS | (reg << 8) |
					       value;
					break;
				    case OUTPUT:
					code = instrOUTPUT_SX_PP | (reg << 8) |
					       value;
					break;
				    case INPUT:
					code = instrINPUT_SX_PP | (reg << 8) |
					       value;
					break;
				    case FETCH:
					code = instrFETCH_SX_SS | (reg << 8) |
					       value;
					break;
				    default:
					error(line,
			...
			default:
				...
				switch (instr) {
				    case ADD:
					code = instrADD_SX_KK | (reg << 8) |
					       (value);
					break;
				    case ADDCY:
					code = instrADDCY_SX_KK | (reg << 8) |
					       (value);
					break;
				    case AND:
					code = instrAND_SX_KK | (reg << 8) |
					       (value);
					break;
				    case COMPARE:
					code = instrCOMPARE_SX_KK | (reg << 8) |
					       (value);
					break;
				    case LOAD:
					code = instrLOAD_SX_KK | (reg << 8) |
					       (value);
					break;
				    case OR:
					code = instrOR_SX_KK | (reg << 8) |
					       (value);
					break;
				    case SUB:
					code = instrSUB_SX_KK | (reg << 8) |
					       (value);
					break;
				    case SUBCY:
					code = instrSUBCY_SX_KK | (reg << 8) |
					       (value);
					break;
				    case TEST:
					code = instrTEST_SX_KK | (reg << 8) |
					       (value);
					break;
				    case XOR:
					code = instrXOR_SX_KK | (reg << 8) |
					       (value);
					break;
				    default:
					error(line, "Unknown instruction");
					return FALSE;
				}
			    } else {
				...
				switch (instr) {
				    case ADD:
					code = instrADD_SX_SY | (reg << 8) |
					       (reg2 << 4);
					break;
				    case ADDCY:
					code = instrADDCY_SX_SY | (reg << 8) |
					       (reg2 << 4);
					break;
				    case AND:
					code = instrAND_SX_SY | (reg << 8) |
					       (reg2 << 4);
					break;
				    case COMPARE:
					code = instrCOMPARE_SX_SY | (reg << 8) |
					       (reg2 << 4);
					break;
				    case LOAD:
					code = instrLOAD_SX_SY | (reg << 8) |
					       (reg2 << 4);
					break;
				    case OR:
					code = instrOR_SX_SY | (reg << 8) |
					       (reg2 << 4);
					break;
				    case SUB:
					code = instrSUB_SX_SY | (reg << 8) |
					       (reg2 << 4);
					break;
				    case SUBCY:
					code = instrSUBCY_SX_SY | (reg << 8) |
					       (reg2 << 4);
					break;
				    case TEST:
					code = instrTEST_SX_SY | (reg << 8) |
					       (reg2 << 4);
					break;
				    case XOR:
					code = instrXOR_SX_SY | (reg << 8) |
					       (reg2 << 4);
					break;
				    default:
					error(line, "Unknown instruction");
...
// bool CAssembler::exportVHDL( string templateFile, string outputDir, string
// entityName)
bool CAssembler::exportVHDL(string templateFile, string outputDir,
                            string entityName, bool bVHDL) {
    int addr, i, j, k, l, n;
    unsigned char INIT[32][64]; /* 32 * 64 = 2048 bytes */
    unsigned char INITP[32][8]; /* 32 * 8 =   256 bytes */
    unsigned int d; /*			 2304 Bytes Total = (18b * 1024 ) / 8 (1 instr = 18
                       bits )*/
    ...
    if (infile == NULL) {
	error(NO_LINE_NR,
	      string("Unable to open template file '" + templateFile + "'")
	          .c_str( ));
	return FALSE;
    }
    ...
    if (outfile == NULL) {
	// RDC 02/02/2007 set VHDL or verilog file extension
	// error( NO_LINE_NR , string( "Unable to open output file '" +
	// exportFile + ".vhd'").c_str() ) ;
	error(NO_LINE_NR, string("Unable to open output file '" + exportFile +
	                         exportExt + "'")
	                      .c_str( ));
	return FALSE;
    }
...
bool CAssembler::assemble( ) {
    ...
    for (it1 = m_registerTable.begin( ); it1 != m_registerTable.end( ); it1++) {
	for (it0 = m_constantTable.begin( ); it0 != m_constantTable.end( );
	     it0++) {
	    if ((*it1)->name == (*it0)->name) {
		cout << "ERROR :: NAMEREG alias:(" << (*it1)->name
		     << ") conflicts with CONSTANT: (" << (*it0)->name << ")\n";
		return FALSE;
	    }
	}
    }
