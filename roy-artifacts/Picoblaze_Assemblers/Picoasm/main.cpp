...
int main(int argc, char **argv) {
	...
	strTemp =
	    strTplFile.substr(strTplFile.size( ) - iCompareLen, iCompareLen);
	...
	if (m_assembler->exportVHDL(strTplFile, strOutputDir, strEntityName,
	                            bVHDL) == true) {
	    if (bVHDL) {
		cout << "Generated VHDL entity file " << strOutputDir << "/"
		     << strEntityName << LA_PICOASM_VHDL_EXT << endl;
	    } else {
		cout << "Generated verilog module file " << strOutputDir << "/"
		     << strEntityName << LA_PICOASM_VERILOG_EXT << endl;
	    }
	} else {
	    if (bVHDL) {
...
bool printListing(string strFileName) {
	...
	*cpExt = '\0';
    ...
    if (fListing == NULL) {
	cout << "ERR: Unable to open assembler listing file '" << strAsmListing
	     << "'" << endl;
	fclose(f);
	return (false);
    }
	...
	if (linenr == iCodeLine) {
	    fprintf(fListing, "%4d  %03x  %05x  %s", linenr + 1, uiAddr,
	            uiHexCode, buf); // one-base line number
	    ...
	    fprintf(fListing, "%4d              %s", linenr + 1,
	            buf); // one-based line number
	}
	linenr++;
    }
